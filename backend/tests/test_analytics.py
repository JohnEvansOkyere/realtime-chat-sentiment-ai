...
class TestChatRoomCreation:
    ...
    def test_create_one_to_one_chat(self, authenticated_client, sample_user):
        ...
        from app.schemas.message import ChatRoomResponse
        
        mock_room = ChatRoomResponse(
            id="new-room-id",
            name=None,
            is_group=False,
        ...
        # Mock the entire chat_service
        mock_service = MagicMock()
        mock_service.create_chat_room = AsyncMock(return_value=mock_room)
...
class TestGetChatHistory:
    ...
    def test_get_messages_not_participant(self, authenticated_client):
        ...
        mock_service.get_chat_history = AsyncMock(
            side_effect=ValueError("User is not a participant of this chat room")
        )
        
        with patch('app.api.chat.chat_service', mock_service):
            response = authenticated_client.get("/api/v1/chat/rooms/some-room-id/messages")
...
class TestGroupManagement:
    ...
    def test_add_participant_not_creator(self, authenticated_client, sample_user):
        ...
        mock_room = {
            "id": "room-id",
            "is_group": True,
            "created_by": "different-user-id"  # Different from sample_user
        }
    ...
    def test_add_participant_success(self, authenticated_client, sample_user):
        ...
        mock_room = {
            "id": "room-id",
            "is_group": True,
            "created_by": sample_user["id"]  # Same as authenticated user
        }
